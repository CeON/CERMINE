...
public final class ParsCitFinalMetadataExtractionEvaluation {
    ...
    private static class PrecisionRecall {
        ...
        public PrecisionRecall buildForList2(List<MetadataList2> metadataList) {
            int precisions = 0;
            int recalls = 0;
            for (MetadataList2 metadata : metadataList) {
                if (metadata.getPrecision() != null) {
                    precision += metadata.getPrecision();
                    precisions++;
                }
    ...
    public void evaluate(NlmIterator iter) throws AnalysisException, IOException, TransformationException, ParserConfigurationException, SAXException, JDOMException, XPathExpressionException, TransformerException {
        ...
        List<MetadataList2> titles = new ArrayList<MetadataList2>();
        ...
        List<MetadataList2> abstracts = new ArrayList<MetadataList2>();
        ...
        for (NlmPair pair : iter) {
            ...
            MetadataList2 title = readMetadataList2(originalNlm, "/article/front/article-meta//article-title",
                                                        extractedNlm, "//algorithm[@name='ParsHed']//title");
            List<String> exp = title.expectedValue;
            List<String> extr = title.extractedValue;
            List<String> exp2 = new ArrayList<String>();
            List<String> extr2 = new ArrayList<String>();
            for (String p : exp) {
                exp2.add(p.replaceAll("[^a-zA-Z0-9]", ""));
            }
            for (String p : extr) {
                extr2.add(p.replaceAll("[^a-zA-Z0-9]", ""));
            }
            ...
            // Abstract
            MetadataList2 abstrakt = readMetadataList2(originalNlm, "/article/front/article-meta/abstract",
        ...
        PrecisionRecall titlePR = new PrecisionRecall().buildForList2(titles);
        titlePR.print("Title");
        ...
        PrecisionRecall abstractPR = new PrecisionRecall().buildForList2(abstracts);
    ...
    private static double calculatePrecision2(List<String> expected, List<String> extracted) {
        if (extracted.isEmpty()) {
            return .0;
        }
        int correct = 0;
        ...
        for (String partExt : extracted) {
            for (String partExp : tmp) {
                if (compareStringsSW(partExp, partExt) >= .9) {
                    ++correct;
                    tmp.remove(partExp);
                    continue external;
                }
    ...
    private static double calculateRecall2(List<String> expected, List<String> extracted) {
        int correct = 0;
        CosineDistance cos = new CosineDistance();
        List<String> tmp = new ArrayList<String>(expected);
        external:
    ...
    private MetadataList2 readMetadataList2(org.w3c.dom.Document originalNlm, String originalXPath, 
            org.w3c.dom.Document extractedNlm, String extractedXPath) throws XPathExpressionException {
        List<String> expected = XMLTools.extractTextAsList(originalNlm, originalXPath);
        List<String> extracted = XMLTools.extractTextAsList(extractedNlm, extractedXPath);
        return new MetadataList2(expected, extracted);
    }
    ...
    private static class MetadataList2 {
        private List<String> expectedValue;
        private List<String> extractedValue;
        private Double precision = -1.;
        private Double recall = -1.;

        public MetadataList2(List<String> expectedValue, List<String> extractedValue) {
            this.expectedValue = expectedValue;
            this.extractedValue = extractedValue;
            this.precision = -1.;
            this.recall = -1.;
        ...
        public Double getPrecision() {
            ...
            return calculatePrecision2(expectedValue, extractedValue);
        }
        ...
        public Double getRecall() {
            ...
            return calculateRecall2(expectedValue, extractedValue);
        }
