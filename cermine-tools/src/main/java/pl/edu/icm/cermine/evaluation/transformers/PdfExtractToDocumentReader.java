 ...
 */
public class PdfExtractToDocumentReader implements FormatToModelReader<Document> {
    ...
    public Document read(Reader reader, Object... hints) throws TransformationException {
        try {
            ...
            XPath xpath = XPath.newInstance("/pdf/title");
            Element title = (Element) xpath.selectSingleNode(root);
            metadata.setTitle(XMLTools.getTextContent(title));
            document.setMetadata(metadata);

            xpath = XPath.newInstance("/pdf/reference");
