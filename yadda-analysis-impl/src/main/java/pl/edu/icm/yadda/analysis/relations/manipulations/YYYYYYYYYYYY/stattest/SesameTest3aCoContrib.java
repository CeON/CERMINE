...
public class SesameTest3aCoContrib extends SesameBasicOperations{

	public static void main(String[] args) throws Exception {
		new SesameTest3aCoContrib().execute();
	}
			...
			for(Map.Entry<String, Set<String>> e : hm.entrySet()){
				...
				for(int i=0;i<arr.length;i++){
					...
					for(int j=i+1;j<arr.length;j++){
						...
						TupleQuery query = this.repository.getConnection().prepareTupleQuery(QueryLanguage.SERQL, contribQuery);
