 ...
 * 
 * Class providing O( N^2 ) clusterizing method.
 * It is simpliest implementation of SingleLinkMethod.
 * Similarity of any object in cluster A to object b is:
 * * similarity of the least similar object in cluster if its less then zero
 * * similarity of the most similar object in cluster if first condition is false  
 * E.g.
 *  X | a | b | c
 *  ---------------
 *  a | X | 4 |-10
 ...
 *  c |-10| 3 | X
 * 
 *  a is the most similar to b, 
 *  b is a little less similar to c, 
 *  but c is not similar with a
 * 
 * a will be merged with b overriding b/c similarity with a/c similarity,
 * because <code>SIM(a,c) < 0 </code>
 * 
 * <code>
 *  X | a | b | c
 *  ---------------
 ...
 *  </code>
 *  
 *  * Same behavior in worser time complexity (O(N^3)) is in {@link PJSimpleHAC_Customized}.
 *  * Behavior which always choose maximum value is implemented in {@link PJSingleLinkHAC}
 *  
 *   
 * @author pdendek
 *
 */
public class PJSingleLinkHAC_Customized implements Clusterizer{

	private static final Logger log = LoggerFactory.getLogger(PJSingleLinkHAC_Customized.class);
...
//			A.add(new SimpleEntry<Double, SimpleEntry<Integer,Integer>>(simil,new SimpleEntry<Integer, Integer>(i1,i2)));
	...
	public Object clone(){
		return new PJSingleLinkHAC_Customized();
	}
}