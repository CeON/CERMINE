...
//	}
	...
	protected static final String REPO_PATH = ("/tmp/repo"+new Date()).replaceAll(" ", "_").replaceAll(":", "_");
	...
	public static void main(String[] args) throws /*Yadda*/Exception, RepositoryException, IOException, RDFHandlerException, RDFParseException, AnalysisException, MalformedQueryException, QueryEvaluationException {
		...
		ICitationParser parser = null; 
		
		InputStream stream = new KimToYExportableTest_MessNotes().getClass().getClassLoader().getResourceAsStream(ITEMS_RESOURCE);
        ...
        File f = new File(("/tmp/"+new Date()).replaceAll(" ", "_").replaceAll(":", "_")+"_clear_repo_after_feeding");
        ...
        SesameRepositoryManipulator srm = new SesameRepositoryManipulator();
        ...
        File f2 = new File(("/tmp/Test+"+new Date()).replaceAll(" ", "_").replaceAll(":", "_")+"po_zamianie_nazwy_person_na_db-person");
		if(f2.exists())f2.delete();
		f2.createNewFile();
		FileOutputStream fos2 = new FileOutputStream(f2);
		RDFWriter qw2 = Rio.createWriter(RDFFormat.N3, fos2);
		rep.getConnection().export(qw2);	
        ...
        File f21 =
//        	new File("/tmp/dodaj");
        	new File(("/tmp/Test+"+new Date()).replaceAll(" ", "_").replaceAll(":", "_")+"po_podaniu_trace");
		...
		repo_file.mkdir();
		NativeStore store2=new NativeStore(repo_file);
		SailRepository rep2=new SailRepository(store2);
		rep2.initialize();
//		rep2.getConnection().add(f21, "", RDFFormat.N3);
		rep2.getConnection().add(new File("/tmp/dodaj"), "", RDFFormat.N3);
		rep.shutDown();
		rep2.shutDown();
        ...
        File f1 = new File(("/tmp/Test+"+new Date()).replaceAll(" ", "_").replaceAll(":", "_")+"na_koniec");
