 ...
 *
 */
public class NewAdaBoostWeightAssignator extends WeightAssignator {
	private static final Logger log = LoggerFactory.getLogger(NewAdaBoostWeightAssignator.class);
	...
	public void assignWeights() throws AnalysisException{
		...
		try{
			...
			// main adaboost loop
			for(int numOfAdaBoostIterations = 0; numOfAdaBoostIterations<maxIterationNumber;numOfAdaBoostIterations++){
					...
					bestFeatureRecord.add(bestFeature);
					
					//recalculate observations weights according to best feature
					//Put is-person-${num} value, where ${num} is order number of person directory starting from 0
					num = 0;
					for(PersonDirectoryCreator p : pdc){
						SesameManipulator sm = manipulators.get("recalculate-weights");
						sm.setRepository(memrepo);
						Map<String, Object> operationParam = new HashMap<String,Object>();
						operationParam.put("trueRelationName", RelConstants.RL_IS_PERSON);
						operationParam.put("testedRelationName", RelConstants.RL_IS_PERSON+"-"+bestFeature);
						sm.execute(operationParam);
						num++;
					}
			}		
