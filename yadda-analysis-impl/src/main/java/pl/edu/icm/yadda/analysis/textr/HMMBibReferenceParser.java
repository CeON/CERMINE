...
public class HMMBibReferenceParser implements BibReferenceParser<BibEntry> {
    ...
    public HMMBibReferenceParser(HMMService hmmService, HMMStorage hmmStorage, String hmmId,
            FeatureVectorBuilder<CitationToken, Citation> featureVectorBuilder) throws IOException {
        this.hmmService = hmmService;
        this.featureVectorBuilder = featureVectorBuilder;
        this.labelProbabilities = hmmStorage.getProbabilityInfo(hmmId);
    }

    public HMMBibReferenceParser(HMMService hmmService, HMMProbabilityInfo<CitationTokenLabel, 
            FeatureVector> labelProbabilities, FeatureVectorBuilder<CitationToken, Citation> featureVectorBuilder) {
        this.hmmService = hmmService;
    ...
    public BibEntry parseBibReference(String text) {
        ...
        for (CitationToken token : tokens) {
            featureVectors.add(featureVectorBuilder.getFeatureVector(token, citation));
        }

        List<CitationTokenLabel> labels = hmmService.viterbiMostProbableStates(labelProbabilities,
                Arrays.asList(CitationTokenLabel.values()), featureVectors);
    ...
    public void setLabelProbabilities(HMMProbabilityInfo<CitationTokenLabel, FeatureVector> labelProbabilities) {
        this.labelProbabilities = labelProbabilities;
    }
