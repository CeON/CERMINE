...
import pl.edu.icm.yadda.analysis.textr.model.BxObject;
import pl.edu.icm.yadda.analysis.textr.model.BxBounds;
import pl.edu.icm.yadda.analysis.textr.model.BxZone;

/**
 * A set-like data structure for objects placed on a plane. Can efficiently find
 * objects in a certain rectangular area. It maintains two parallel lists of
 * objects, each of which is sorted by its x or y coordinate.
 * 
...
class DocumentPlane {
	...
	/**
	 * Size of a grid square. If gridSize=50, then the plane is divided into
	 * squares of size 50. Each square contains objects placed in a 50x50 area
	 */
	private Integer gridSize;
	/**
	 * Redundant dictionary of objects on the plane. Allows efficient 2D space
	 * search. Keys are X-Y coordinates of a grid square. Single object can be
	 * stored under several keys (depending on its physical size). Grid squares
	...
	public DocumentPlane add(BxObject obj) {
		...
		/*
		 * object list must contain the same number of objects as object
		 * dictionary
		 */
		...
		for(int y = y0.intValue()/gridSize; y < ((int)(y1+gridSize-1))/gridSize; ++y)
			for(Integer x = x0.intValue()/gridSize; x < ((int)(x1+gridSize-1))/gridSize; ++x) {
				...
				for(BxObject obj: grid.get(xy)) {
					if(done.contains(obj)) /* omit if already checked */
						continue;
